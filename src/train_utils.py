...
def configuration(FLAGS):
    save_dir = '/workspace/Model/'+FLAGS.model+'/'+FLAGS.dataset+'/ensemble/'+FLAGS.q_mode+'/'+str(FLAGS.num_bit)+'_'+str(FLAGS.num_bit+FLAGS.up_bit)+'bit/'
    save_file = save_dir+FLAGS.model+'_ext.ckpt'
    logdir = '/workspace/logs/'+FLAGS.q_mode+'/'+FLAGS.dataset+'/ensemble/'+FLAGS.model+'/'+str(FLAGS.num_bit)+'to'+str(FLAGS.num_bit+FLAGS.up_bit)+'bit'
    Figname = "/workspace/figure/"+FLAGS.dataset+"/"+FLAGS.q_mode+"_"+FLAGS.model+"_"+str(FLAGS.num_bit)+'_'+str(FLAGS.num_bit+FLAGS.up_bit)+"bit.png"
    log_save = '/workspace/history/'+FLAGS.dataset+'/'+FLAGS.q_mode+'/'+FLAGS.model+'/'+str(FLAGS.num_bit)+'to'+str(FLAGS.num_bit+FLAGS.up_bit)+'bit.txt'
    ...
    log_manager.print_configuration(FLAGS)
...
def load_model(FLAGS, Placeholder):
    ...
    for gpu_id in range(FLAGS.gpu_nums):
            with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_id)):
                with tf.variable_scope(tf.get_variable_scope(), reuse=(gpu_id > 0)):
                    ...
                    #         BLOCK = [3, 8, 36, 3]
                    #     fix_model = ResNet50(X_A[gpu_id], qnum=FLAGS.num_bit, block=BLOCK,
                    ...
                    tr_hypothesis = Model.upgrade(X_A[gpu_id])
                    tr_cost = tf.nn.softmax_cross_entropy_with_logits(
                        logits=tr_hypothesis,
                        labels=Y_A[gpu_id])

                    add_hypothesis = (tf.divide(tr_hypothesis,FLAGS.factor)+ fix_hypothesis)/2.0
                    shr_cost = tf.nn.softmax_cross_entropy_with_logits(
                        logits=add_hypothesis,
                        labels=Y_A[gpu_id])         
                    fix_losses.append(fix_cost)                      
                    tr_losses.append(tr_cost)
                    shr_losses.append(shr_cost)
                    ...
                    tr_correct_prediction = tf.equal(tf.argmax(tr_hypothesis, 1), tf.arg_max(Y_A[gpu_id], 1))
                    tr_correct.append(tr_correct_prediction)

    # fix_loss = tf.reduce_mean(tf.concat(fix_losses, axis=0))
    tr_loss = tf.reduce_mean(tf.concat(tr_losses, axis=0))
    shr_loss = tf.reduce_mean(tf.concat(shr_losses, axis=0))
    ...
    add_accuracy = tf.reduce_mean(tf.cast(tr_correct, tf.float32) , name='add_accuracy')
    add_acc_sum = tf.summary.scalar("add_accuarcy", add_accuracy)

    var_all = tf.trainable_variables(scope=None)
    var_m1 = tf.trainable_variables(scope='Model')
    var_only_m2 = tf.trainable_variables(scope='Train')

    return (tr_loss, shr_loss), (fix_accuracy, add_accuracy), (var_all, var_m1, var_only_m2)

def make_optimizer(placeholder, loss, var_list):
    _, _, _, learning_rate = placeholder
    tr_loss, shr_loss = loss
    var_all, var_m1, var_only_m2 = var_list

    fix_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(shr_loss, 
                                            var_list=var_m1, colocate_gradients_with_ops=True)
    add_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(shr_loss, 
                                            var_list=var_all, colocate_gradients_with_ops=True)
    stop_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(tr_loss, 
                                            var_list=var_only_m2, colocate_gradients_with_ops=True)

    return fix_optimizer, add_optimizer, stop_optimizer
...
def fit_model(sess, optimizer, placeholder, iterator, train_ds, step, FLAGS, log_manager, summary_opreation=None, writer=None):
    ...
    if step <= FLAGS.stop_point:
        if step % FLAGS.iteration == 0:
            lr = FLAGS.add_lr
        elif step >= 20:
            lr = lr / 2
            if step >= 30:
